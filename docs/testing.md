...
## v_bus_test — OBC ↔ ADCS Communication Sanity Check

### What it is

`tests/test_v_bus.c` is a small, self-contained check that the v-bus transport
(`platform/sim/spi/v_bus.c`) actually works — the same code `obc_sim` and
`adcs_sim` use to talk to each other over the simulated SPI bus (a Unix domain
socket at `/tmp/v_bus.sock`).

It does **not** mock anything. It forks two processes in-test — one calling
`v_bus_initialize(1)` (OBC/master role) and one calling `v_bus_initialize(0)`
(ADCS/slave role) — and drives the real `send()`/`receive()` functions between
them, exactly like the two real binaries do. If the master/slave handshake, the
socket, or the framing breaks, this test breaks with it.
...
On 2026-08-04 the v-bus silently broke on macOS: a socket type
(`SOCK_SEQPACKET`) that isn't supported there caused `v_bus_initialize()` to
fail immediately, but nothing checked the return value — so both `obc_sim` and
`adcs_sim` kept running with a dead bus and no error message. `adcs_sim` looked
fine (its Control loop doesn't use the bus), `obc_sim` looked "frozen." This
test exists so that specific failure mode — and anything like it — shows up as
an immediate, obvious `FAIL` instead of hours of silent debugging.
...
```bash
cmake --build build --target v_bus_test && ./build/tests/v_bus_test
```
...
```
[V-BUS] Slave connecting..
[V-BUS] Master waiting for Slave...
[V-BUS] Slave connected to Master.
[V-BUS] Master accepted connection from Slave.
...
```
[ADCS] FAIL: v_bus_initialize failed
[OBC] FAIL: v_bus_initialize failed
v_bus_test: FAIL (slave_ok=0 master_ok=0)
```
...
| Message | Meaning |
|---|---|
| `v_bus_initialize failed` | `socket()`/`bind()`/`listen()`/`accept()`/`connect()` failed — check the `[V-BUS] ... failed: <reason>` line printed just above it for the actual `errno` string. |
...
holding `/tmp/v_bus.sock` (see below), then re-run.
...
- **Don't run it alongside the real binaries.** It uses the exact same
  hardcoded socket path (`/tmp/v_bus.sock`) as `obc_sim`/`adcs_sim`. Running
  it at the same time as either of those will cause spurious failures on
  both sides. If you get a confusing failure, check for stragglers:
  ```bash
...
- **SIM-only.** The test is only built when `HW_MODE=OFF` (the default) —
  there's no real SPI hardware to test against in `HW_MODE=ON` builds yet.
...
- **When to run it:** any time you touch `platform/sim/spi/v_bus.c`, or
  whenever `obc_sim`/`adcs_sim` seem to not be talking to each other and you
  want to know in 5 seconds whether the bus itself is the problem before
